...
- **Governance Phase 8:** Guide Review Voting Lite. Owner/admin/reviewer roles can vote on guides with approve or request_changes. Votes use network_role_definitions weight. UI displays vote summary and weights. Does not auto-publish and does not enforce thresholds yet. Data only, no RLS enforcement.
